export type FocusArea = "leasing" | "factoring" | "prestamos" | "tarjetas" | "pension" | "workflow" | "otros";

export interface ReferenceItem {
  name: string;
  region: string;
  deployment: string;
  product: string;
  detail: string;
  result: string;
  modules?: string;
  contact?: string;
  web?: string;
  inImplementation?: boolean;
  focus: FocusArea[];
  core: "SYSDE PLUS" | "FileMaster";
}

export const focusLabels: Record<FocusArea, string> = {
  leasing: "Leasing",
  factoring: "Factoring",
  prestamos: "Préstamos",
  tarjetas: "Tarjetas",
  pension: "Fondos de Pensión",
  otros: "Otros",
};

export const references: ReferenceItem[] = [
  {
    name: "Grupo CMI",
    region: "🇬🇹🇸🇻🇭🇳🇳🇮🇨🇷🇵🇦🇲🇽🇺🇸🇩🇴🇪🇨🇨🇴🇵🇪🇪🇸🇵🇹🇮🇹🇫🇷🇬🇧 +20 países",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Corporación Multi Inversiones (CMI) es uno de los conglomerados empresariales privados más grandes de Centroamérica, con más de 54,000 colaboradores y operaciones en más de 20 países.\n\nDesde finales del 2025, SYSDE realiza la implementación de SYSDE PLUS sobre la operación de Factoring y Leasing de Grupo CMI, estandarizando procesos multi-país y multi-moneda bajo un modelo modular.\n\nLa plataforma SYSDE PLUS maneja y controla el ciclo end-to-end de Factoring y Leasing.\n\nAdemás, con el core de API's de SYSDE se realizará la integración con 4 cores de SAP.\n\nPresencia: Centroamérica (Guatemala, El Salvador, Honduras, Nicaragua, Costa Rica, Panamá) · Norteamérica (México, Estados Unidos) · Caribe (República Dominicana) · Sudamérica (Ecuador, Colombia, Perú) · Europa (España, Portugal, Italia, Francia, Reino Unido).",
    result: "En ejecución – Plataforma end-to-end para Factoring, Leasing y Crédito Puente con integración a 4 cores SAP.",
    modules: "Factoring, Leasing, Crédito Puente",
    web: "https://somoscmi.com/es/",
    inImplementation: true,
    focus: ["factoring", "leasing", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Arrendadora CREMI (Grupo BAL / GNP Seguros)",
    region: "🇲🇽 México",
    deployment: "On-Premise",
    product: "SYSDE PLUS",
    detail: "Constituida en 1975 como Arrendadora CREMI S.A. de C.V., O.A.C., perteneciente al Grupo BAL — conglomerado diversificado con presencia en los sectores comercial, agropecuario, industrial, financiero, salud y educación en México.\n\nIngresos: $11,129 M USD (2020)\nActivos: $25,973 M USD (2020)\nEmpleados: 42,850\nSede Central: Ciudad de México\n\nEn el año 2000 se convierte en subsidiaria de GNP Seguros y realiza un cambio de core tecnológico, seleccionando SYSDE PLUS como plataforma central para arrendamientos y factoraje.",
    result: "Migración exitosa de core tecnológico a SYSDE PLUS para operaciones de leasing y factoraje dentro del Grupo BAL.",
    modules: "Leasing, Factoring",
    web: "https://www.bal.com.mx",
    focus: ["leasing", "factoring"],
    core: "SYSDE PLUS",
  },
  {
    name: "AMC",
    region: "🇸🇻 El Salvador",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Desde agosto de 2025, implementación de SYSDE PLUS: plataforma modular (modelo todo incluido) end-to-end, multimoneda con trazabilidad total, alineada a nuevas regulaciones.",
    result: "En ejecución – Adaptado a regulaciones que entrarán en vigencia en El Salvador.",
    web: "https://amc.com.sv",
    inImplementation: true,
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Dos Pinos",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS optimizando operaciones con módulos de Seguridad, Clientes, Contabilidad, Reportes SICVECA, Préstamos, Líneas de Crédito. Integración con SAP.",
    result: "+200 puntos de venta en Centroamérica, +70 años de experiencia, +10M consumidores.",
    modules: "Seguridad, Clientes, Contabilidad, SICVECA, Préstamos, Líneas de Crédito",
    contact: "Kelvin Calvo Brown – Consultor de Estrategia TI",
    inImplementation: true,
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Grupo Apex",
    region: "🇬🇹 Guatemala",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS para operaciones de Leasing y Préstamos, estandarizando procesos operativos y financieros.",
    result: "En ejecución – Implementación activa de módulos de Leasing y Préstamos.",
    modules: "Leasing, Préstamos",
    inImplementation: true,
    focus: ["leasing", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Grupo Aurum",
    region: "🇬🇹 Guatemala",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS para operaciones de Leasing y Préstamos, cubriendo el ciclo end-to-end de ambas líneas de negocio.",
    result: "En ejecución – Implementación activa de módulos de Leasing y Préstamos.",
    modules: "Leasing, Préstamos",
    inImplementation: true,
    focus: ["leasing", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Unicomer Caribbean Holding",
    region: "🇸🇻🇬🇹🇭🇳🇳🇮🇨🇷🇵🇦🇩🇴🇯🇲🇹🇹🇧🇧🇨🇼🇦🇼🇪🇨🇵🇾🇺🇸 +19 países",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "SYSDE ejecutó con éxito la implementación de los sistemas Préstamos y Tarjetas, adaptando una amplia gama de módulos: Cobranzas, Cuentas de Efectivo y Corrientes, Configuración y Parámetros, Clientes, Créditos (incluido Crédito Retail), Depósitos a Plazo, Cajas, Contabilidad, Bancos, IVA, Lavado de Activos, Central de Riesgos, Gestiones, Activos Fijos, Inversiones, Custodia de Valores, Accionistas, Compras, Sistemas de Información Gerencial y Presupuesto.\n\nEsta implementación comenzó en 2008, con expansiones y adecuaciones para El Salvador en 2011 y Nicaragua en 2014, donde actualmente continúan operando y evolucionando a nivel operativo y normativo.\n\nDesde el año 2000, Grupo Unicomer ha gestionado más de 30 marcas y una red de más de 1,200 tiendas en 26 países de Centroamérica, Sudamérica, el Caribe y Estados Unidos, con más de 13,000 colaboradores.",
    result: "Implementación exitosa desde 2008 con expansiones a El Salvador (2011) y Nicaragua (2014). +30 marcas, +1,200 tiendas en 26 países, +13,000 colaboradores.",
    modules: "Cobranzas, Créditos, Depósitos, Contabilidad, PLD, Inversiones, Tarjetas",
    contact: "Guillermo J. Siman – Vice Chairman",
    focus: ["tarjetas", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Financiera MultiMoney",
    region: "🌎 Centroamérica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS. Contribuimos a lograr su visión de crear productos eficientes, transparentes y honestos.",
    result: "Exitoso – Más de 10 años en operación. Opera a nivel regional con Akros como Corporativo Tecnológico.",
    modules: "Seguridad, Préstamos, Cobranza, Ahorros, Depósitos, Contabilidad, PLD, Factoraje",
    contact: "Ernesto Fernández Lang – Presidente",
    web: "https://multimoney.com/",
    focus: ["prestamos", "factoring"],
    core: "SYSDE PLUS",
  },
  {
    name: "ION",
    region: "🇲🇽 México",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación exitosa de SYSDE PLUS para industria inmobiliaria: crédito puente, financiamiento de vivienda. App móvil para avance de obras.",
    result: "+1,090 sueños de vivienda cumplidos, +$15 mil MDP en construcción, +10 mil viviendas financiadas.",
    contact: "Edgar Rojas Flores – Líder nSAF",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Factor y Valor",
    region: "🇨🇴 Colombia",
    deployment: "SaaS",
    product: "SYSDE PLUS (Factoring)",
    detail: "Administración de cartera de Factoring. Más de una década como aliado financiero de pymes colombianas.",
    result: "Exitoso – Operación de Factoring para pymes colombianas con soluciones de capital de trabajo.",
    contact: "Jorge Ivan Bedoya Correa – Director General",
    focus: ["factoring"],
    core: "SYSDE PLUS",
  },
  {
    name: "Broxel",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS como core central para Banca Digital. Broxel lanzó una de las primeras Super Apps en México.",
    result: "Primera Fintech Digital en México. Tarjetas Débito/Crédito en entorno digital (México y USA).",
    contact: "Rodrigo Díaz – VP Operaciones",
    focus: ["tarjetas"],
    core: "SYSDE PLUS",
  },
  {
    name: "Bankaool",
    region: "🇲🇽 México",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación y Evolución de SYSDE PLUS. Desde cooperativa hasta Banco de primer piso. Banca 100% digital 24x7.",
    result: "+25 años en operación. +40 años de historia. Licencia bancaria desde 2019.",
    contact: "Randall González – CTO",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  // ── Pensiones (ordenadas por tamaño de fondos, mayor a menor) ──
  {
    name: "Afore XXI Banorte",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de retiro con SYSDE PLUS.",
    result: "Afiliados: 7.2 millones. Fondos: $49,597 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Porvenir",
    region: "🇨🇴 Colombia",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 14,842,405. Fondos: $47,000 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Alcatel-Lucent Enterprise",
    region: "🇫🇷 Francia",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos previsionales con SYSDE PLUS. Hoy es Nokia.",
    result: "Afiliados: 78,400. Fondos: $40.67 B.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Banorte (Pensiones)",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de pensiones con SYSDE PLUS.",
    result: "Afiliados: 12 M. Fondos: $39,633 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Afore Pensionissste Contigo",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de retiro con SYSDE PLUS.",
    result: "Afiliados: 1 millón. Fondos: $27,000 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "ProFuturo (México)",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de retiro con SYSDE PLUS.",
    result: "Afiliados: 1.7 M. Fondos: $26,300 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Integración AFAP",
    region: "🇺🇾 Uruguay",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión previsional con SYSDE PLUS.",
    result: "Afiliados: 1,654,967. Fondos: $22,103 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "República AFAP",
    region: "🇺🇾 Uruguay",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 1.6 millones. Fondos: $22,000 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Futuro de Bolivia AFP",
    region: "🇧🇴 Bolivia",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión previsional con SYSDE PLUS.",
    result: "Afiliados: 500,000. Fondos: $20,998 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Prima AFP",
    region: "🇵🇪 Perú",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones del Grupo Crédito con SYSDE PLUS.",
    result: "Afiliados: 2.3 millones. Fondos: $13,078 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Colfondos",
    region: "🇨🇴 Colombia",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones del Grupo Habitat con SYSDE PLUS.",
    result: "Afiliados: 5.2 M. Fondos: $12,500 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Principal",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión previsional con SYSDE PLUS.",
    result: "Afiliados: 3,085,011 M. Fondos: $12,500 M.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "MetLife",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 698,784. Fondos: $12 mil M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFAP SURA",
    region: "🇺🇾 Uruguay",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión previsional con SYSDE PLUS.",
    result: "Afiliados: 7.8 millones. Fondos: $11,512 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Popular Pensiones",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 1,728,132. Fondos: $10,568 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Inbursa Afore",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de retiro con SYSDE PLUS.",
    result: "Afiliados: 1.1 millones. Fondos: $9,291 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Confía",
    region: "🇸🇻 El Salvador",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS desde la fundación en 1998. Líderes en fondos previsionales en Centroamérica y el Caribe.",
    result: "Afiliados: 1.8 millones. Fondos: $8,219 M USD.",
    contact: "Luis Diego Varaona Magaña – Dir. Operaciones y Tecnología",
    web: "https://www.confia.com.sv/",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Popular",
    region: "🇩🇴 República Dominicana",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 1,644,169. Fondos: $6,576 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Profuturo",
    region: "🇵🇪 Perú",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 1.7 M. Fondos: $6,503 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "BN Vital",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 540,946. Fondos: $4,981 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Crecer",
    region: "🇸🇻 El Salvador",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 1.5 millones. Fondos: $4,505 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Siembra",
    region: "🇪🇨 Ecuador",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 1.1 millones. Fondos: $4,228 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Pensiones BAC Credomatic",
    region: "🌎 Centroamérica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 472,148. Fondos: $3,969 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Habitat",
    region: "🇵🇪 Perú",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 1 millón. Fondos: $3,942 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Unión Capital AFAP",
    region: "🇺🇾 Uruguay",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 375,255. Fondos: $3,789 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Reservas",
    region: "🇩🇴 República Dominicana",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 724,504. Fondos: $3,409 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "RAP",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 273,101. Fondos: $1,540 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Atlántida",
    region: "🇭🇳 Honduras",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de pensiones y cesantías con SYSDE PLUS.",
    result: "Afiliados: 78,000. Fondos: $1,157 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Compañía de Seguros (México)",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 587,097. Fondos: $821 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Petros",
    region: "🇧🇷 Brasil",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 220,000. Fondos: $784 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFP Atlántico",
    region: "🇭🇳 Honduras",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 104,047. Fondos: $382.7 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "AFPC Occidente",
    region: "🇬🇹 Guatemala",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 82,539. Fondos: $161.5 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Mercantil",
    region: "🇻🇪 Venezuela",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión de fondos previsionales con SYSDE PLUS.",
    result: "Afiliados: 100,007. Fondos: $150.28 M.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Seguros Universal",
    region: "🇩🇴 República Dominicana",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Gestión previsional con SYSDE PLUS.",
    result: "Afiliados: 38,498. Fondos: $95.1 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Tokio Marine Seguradora",
    region: "🇧🇷 Brasil",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS.",
    result: "Afiliados: 52,000. Fondos: $37.1 M USD.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  {
    name: "Hanwha Life (한화생명)",
    region: "🇰🇷 Corea del Sur",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Administración de fondos de pensiones con SYSDE PLUS para una de las principales aseguradoras de Corea del Sur.",
    result: "Operación activa en gestión previsional internacional.",
    focus: ["pension"],
    core: "SYSDE PLUS",
  },
  // ── Otros clientes ──
  {
    name: "Banco ADOPEM",
    region: "🇩🇴 República Dominicana",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "SYSDE PLUS con 12 módulos. Institución orientada a finanzas productivas para sectores no bancarizados.",
    result: "Parte del Grupo Fundación Microfinanzas BBVA. Impacto en inclusión financiera.",
    contact: "Mercedes Canalda – Presidente Ejecutiva",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "COOPECAR R.L.",
    region: "🇨🇷 Costa Rica",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS (Cooperativas)",
    detail: "Sistema SYSDE PLUS para Cooperativas. Cuentas de Efectivo, Depósitos, Préstamos, Contabilidad, Cajas, SICVECA.",
    result: "Cliente activo desde 2006. Mantenimiento continuo y actualizaciones regulatorias.",
    modules: "Cuentas de Efectivo, Depósitos, Préstamos, Contabilidad, Cajas, SICVECA",
    contact: "MBA Carmen Milena Arce Alfaro – Gerente General",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "SOFIMSA",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Servicio SaaS en Microsoft Azure de Factoring administrado por SYSDE PLUS.",
    result: "Administración exitosa de cartera de Factoring con soluciones de capital de trabajo.",
    focus: ["factoring"],
    core: "SYSDE PLUS",
  },
  {
    name: "CCSS",
    region: "🇨🇷 Costa Rica",
    deployment: "On Premise / SaaS",
    product: "FileMaster",
    detail: "Migración exitosa de expedientes digitales de FileMaster con +6,120 usuarios activos hacia la nube.",
    result: "Optimización de gestión, seguridad y accesibilidad con infraestructura escalable.",
    focus: ["otros"],
    core: "FileMaster",
  },
  {
    name: "Financiera MVA",
    region: "🇬🇹 Guatemala",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS apoyando el crecimiento de PyMEs guatemaltecas.",
    result: "Implementación exitosa con módulos completos de la plataforma.",
    modules: "Seguridad, Préstamos, Cobranza, Ahorros, Depósitos, Contabilidad, PLD, Factoraje",
    contact: "Mario Ernesto López Pineda – Gerente General",
    web: "https://www.financieramva.com/",
    focus: ["prestamos", "factoring"],
    core: "SYSDE PLUS",
  },
  {
    name: "CFE Rwanda",
    region: "🇷🇼 Ruanda",
    deployment: "SaaS",
    product: "SAF",
    detail: "En 2004, implementación de SAF para automatizar contabilidad, préstamos y arrendamiento.",
    result: "En 2015, Bank of Africa adquirió 90% de participación. Transformación a banco comercial.",
    focus: ["prestamos", "leasing"],
    core: "SYSDE PLUS",
  },
  {
    name: "CNEC Djibouti",
    region: "🇩🇯 Yibuti",
    deployment: "SaaS",
    product: "SAF",
    detail: "Implementación de SAF en 2009 para optimizar operaciones financieras para grupos vulnerables.",
    result: "En 2011, fusión con CPEC. Impacto en inclusión financiera de mujeres emprendedoras.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Nyogondemeso-Soba",
    region: "🇲🇱 Mali",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de SYSDE PLUS tras fusión de dos cooperativas. Ahorro, crédito y transferencias.",
    result: "Fusión exitosa. Gobernanza democrática y apoyo a lucha contra la pobreza.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "CPECG",
    region: "🇬🇳 Guinea",
    deployment: "SaaS",
    product: "SAF",
    detail: "Implementación de SAF para gestión financiera. +150,000 clientes, 27 sucursales.",
    result: "Reconocimiento por desempeño en microfinanzas e inclusión financiera.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "CTI",
    region: "🇸🇳 Senegal",
    deployment: "SaaS",
    product: "SAF",
    detail: "Informatización de 72 Systèmes Financiers Décentralisés (SFD) en Senegal.",
    result: "72 instituciones financieras descentralizadas informatizadas exitosamente.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Dunduliza",
    region: "🇹🇿 Tanzania",
    deployment: "SaaS",
    product: "SAF",
    detail: "Fortalecimiento de cooperativas de ahorro y crédito (SACCOs) en Tanzania.",
    result: "Mejora en capacidad operativa. Fomento del desarrollo económico local.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "FCPB",
    region: "🇧🇫 Burkina Faso",
    deployment: "SaaS",
    product: "SAF",
    detail: "Transformación digital de la red de microfinanzas más grande de Burkina Faso. Fundada en 1972.",
    result: "+1,200 empleados, 455 dirigentes. Inclusión financiera exitosa.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "FCRMD",
    region: "🇲🇱 Mali",
    deployment: "SaaS",
    product: "SAF",
    detail: "Modernización de cooperativas rurales mutualistas del Delta en Mali.",
    result: "Inclusión económica de mujeres y jóvenes en áreas rurales.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Credix",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de crédito y financiamiento en Costa Rica.",
    result: "Operación activa con módulos de préstamos y gestión de cartera.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "SYSRETAIL",
    region: "🌎 Latinoamérica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma especializada en servicios financieros para el sector retail, incluyendo crédito al consumo y tarjetas privadas.",
    result: "Operación activa en múltiples mercados de la región.",
    focus: ["tarjetas", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Banco Promerica",
    region: "🇬🇹🇸🇻🇭🇳🇳🇮🇨🇷🇵🇦🇩🇴🇪🇨 Centroamérica",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de soluciones SYSDE para operaciones bancarias en múltiples países de Centroamérica.",
    result: "Operación activa a nivel regional con presencia en 8+ países.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "GMAC Financial Services",
    region: "🇲🇽 México",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Soluciones de financiamiento y leasing automotriz con SYSDE PLUS.",
    result: "Operación activa en financiamiento automotriz.",
    focus: ["leasing", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Worldwide Foundation for Credit Unions",
    region: "🌎 Internacional",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Soporte tecnológico a cooperativas de ahorro y crédito a nivel mundial.",
    result: "Fortalecimiento de cooperativas financieras en mercados emergentes.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Banco de Bogotá",
    region: "🇨🇴 Colombia",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de soluciones SYSDE para operaciones de leasing y financiamiento empresarial.",
    result: "Operación activa en uno de los bancos más grandes de Colombia.",
    focus: ["leasing", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Credicomer",
    region: "🇸🇻 El Salvador",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para créditos y operaciones financieras.",
    result: "Operación activa con módulos de préstamos y plataforma SYSDE PLUS.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Davivienda",
    region: "🇨🇴 Colombia",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Soluciones SYSDE implementadas para operaciones financieras de uno de los principales bancos de Colombia.",
    result: "Operación activa con presencia en múltiples países de la región.",
    focus: ["prestamos", "workflow"],
    core: "SYSDE PLUS",
  },
  {
    name: "DOM",
    region: "🇩🇴 República Dominicana",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de préstamos y financiamiento.",
    result: "Operación activa en República Dominicana.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "abcdin",
    region: "🇨🇱 Chile",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma de crédito al consumo y tarjetas para retail financiero en Chile.",
    result: "Operación activa en uno de los principales retailers financieros de Chile.",
    focus: ["tarjetas", "prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Banpro (Grupo Promerica)",
    region: "🇳🇮 Nicaragua",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de soluciones SYSDE para el banco más grande de Nicaragua.",
    result: "Operación activa con módulos de préstamos en SYSDE PLUS.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "BANDESAL",
    region: "🇸🇻 El Salvador",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Banco de Desarrollo de El Salvador con SYSDE PLUS para gestión de préstamos de desarrollo económico.",
    result: "Operación activa apoyando el desarrollo económico de El Salvador.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Credex Microfinanzas",
    region: "🇳🇮 Nicaragua",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de microcrédito e inclusión financiera.",
    result: "Operación activa en microfinanzas en Nicaragua.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Banco Nacional",
    region: "🇨🇷 Costa Rica",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Soluciones SYSDE implementadas en el banco estatal más grande de Costa Rica.",
    result: "Operación activa con módulos de préstamos en SYSDE PLUS.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "CrediCefi",
    region: "🇬🇹 Guatemala",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de crédito y financiamiento en Guatemala.",
    result: "Operación activa con módulos de préstamos.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Scotiabank",
    region: "🇨🇷 Costa Rica",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de soluciones SYSDE para operaciones de Scotiabank en la región.",
    result: "Operación activa con SYSDE PLUS.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "FAFIDESS",
    region: "🇬🇹 Guatemala",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de microfinanzas e inclusión financiera en Guatemala.",
    result: "Operación activa en microfinanzas.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "FUNDEVI",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Fundación para el Desarrollo de la Vivienda con SYSDE PLUS para gestión de préstamos habitacionales.",
    result: "Operación activa en financiamiento de vivienda en Costa Rica.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Capital Activo",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Sociedad Financiera Popular con SYSDE PLUS para préstamos y factoraje.",
    result: "Operación activa en financiamiento para PyMEs mexicanas.",
    focus: ["prestamos", "factoring"],
    core: "SYSDE PLUS",
  },
  {
    name: "Desjardins",
    region: "🇨🇦 Canadá",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Implementación de soluciones SYSDE para el grupo cooperativo financiero más grande de Canadá.",
    result: "Operación activa en gestión de cooperativas financieras.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Unión de Crédito Progreso",
    region: "🇲🇽 México",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de crédito y financiamiento empresarial.",
    result: "Operación activa en financiamiento empresarial en México.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Banco Santa Cruz",
    region: "🇧🇴 Bolivia",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Soluciones SYSDE para operaciones bancarias en Bolivia.",
    result: "Operación activa con módulos de préstamos en SYSDE PLUS.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "BN Valores",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE para gestión de inversiones y valores del Banco Nacional de Costa Rica.",
    result: "Operación activa en gestión de inversiones.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Bancard",
    region: "🇵🇾 Paraguay",
    deployment: "On Premise / SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE para procesamiento de tarjetas y pagos electrónicos en Paraguay.",
    result: "Operación activa como procesador de pagos líder en Paraguay.",
    focus: ["tarjetas"],
    core: "SYSDE PLUS",
  },
  {
    name: "Fundap",
    region: "🇬🇹 Guatemala",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de microfinanzas y desarrollo comunitario en Guatemala.",
    result: "Operación activa en microfinanzas e inclusión financiera.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
  {
    name: "Navix Sociedad Financiera",
    region: "🇨🇷 Costa Rica",
    deployment: "SaaS",
    product: "SYSDE PLUS",
    detail: "Plataforma SYSDE PLUS para operaciones de crédito y financiamiento en Costa Rica.",
    result: "Operación activa con módulos de préstamos.",
    focus: ["prestamos"],
    core: "SYSDE PLUS",
  },
];
