...
export const questions: QuestionItem[] = [
  ...
  // ── Arquitectura (H) ──
  {
    ...
    respuesta: "𝗥𝗲𝘀𝗽𝘂𝗲𝘀𝘁𝗮: Sí, SYSDE PLUS cuenta con arquitectura de integración robusta que soporta APIs RESTful y eventos.\n\n𝗔𝗣𝗜𝘀 𝗥𝗘𝗦𝗧𝗳𝘂𝗹:\n• +200 endpoints preconstruidos documentados cubriendo todo el ciclo de vida\n• Documentación OpenAPI/Swagger disponible\n• Versionamiento para compatibilidad hacia atrás\n• Autenticación OAuth 2.0 / JWT\n\n𝗜𝗻𝘁𝗲𝗴𝗿𝗮𝗰𝗶ó𝗻 𝗽𝗼𝗿 𝗘𝘃𝗲𝗻𝘁𝗼𝘀:\n• Publicación y suscripción de eventos de negocio (event-driven architecture)\n• Webhooks configurables para cambios de estado en operaciones\n• Compatible con Apache Kafka, RabbitMQ y Azure Service Bus\n\n𝗜𝗻𝘁𝗲𝗴𝗿𝗮𝗰𝗶𝗼𝗻𝗲𝘀 𝗧í𝗽𝗶𝗰𝗮𝘀:\n• CRM → solicitudes y sincronización de datos\n• ERP/SAP → asientos contables y catálogos (CMI: 4 instancias SAP)\n• Pricing → consulta de tasas externas\n• Data Warehouse → extracción de datos vía APIs o vistas de BD\n• Canales Digitales → consulta de estados y saldos desde portal/app",
    respuesta_en: "𝗔𝗻𝘀𝘄𝗲𝗿: Yes, SYSDE PLUS features robust integration architecture supporting RESTful APIs and events.\n\n𝗥𝗘𝗦𝗧𝗳𝘂𝗹 𝗔𝗣𝗜𝘀:\n• +200 pre-built documented endpoints covering the entire lifecycle\n• OpenAPI/Swagger documentation available\n• Versioning for backward compatibility\n• OAuth 2.0 / JWT authentication\n\n𝗘𝘃𝗲𝗻𝘁-𝗗𝗿𝗶𝘃𝗲𝗻 𝗜𝗻𝘁𝗲𝗴𝗿𝗮𝘁𝗶𝗼𝗻:\n• Business event pub/sub (event-driven architecture)\n• Configurable webhooks for operation state changes\n• Compatible with Apache Kafka, RabbitMQ, and Azure Service Bus\n\n𝗧𝘆𝗽𝗶𝗰𝗮𝗹 𝗜𝗻𝘁𝗲𝗴𝗿𝗮𝘁𝗶𝗼𝗻𝘀:\n• CRM → applications and data sync\n• ERP/SAP → accounting entries and catalogs (CMI: 4 SAP instances)\n• Pricing → external rate queries\n• Data Warehouse → data extraction via APIs or DB views\n• Digital Channels → status and balance queries from portal/app",
  ...
  // ── Transparencia (M) ──
  {
    ...
    respuesta: "La solución de leasing de SYSDE PLUS cumple con todo lo requerido para manejar una operación del nivel de BCP. La plataforma cubre el ciclo completo end-to-end con la robustez necesaria para entidades reguladas.\n\nSYSDE nunca se detiene — contamos con un roadmap evolutivo exclusivo para BCP, sin costo adicional, que garantiza que la plataforma evolucione continuamente sin afectar la operación normal.",
    respuesta_en: "SYSDE PLUS's leasing solution complies with everything required to manage a BCP-level operation. The platform covers the complete end-to-end cycle with the robustness needed for regulated entities.\n\nSYSDE never stops — we have an exclusive evolution roadmap for BCP, at no additional cost, ensuring the platform evolves continuously without affecting normal operations.",
    valor: "La transparencia de SYSDE sobre la evolución del producto, combinada con un roadmap sin costo adicional, genera confianza y demuestra compromiso con BCP.",
    valor_en: "SYSDE's transparency about product evolution, combined with a no-additional-cost roadmap, builds trust and demonstrates commitment to BCP.",
    diagrams: [
      { type: "interactive-roadmap" as DiagramType },
    ],
    diagrams_en: [
