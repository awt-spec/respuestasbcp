        ...
        {subs.map((sub, i) => {
          ...
          return (
            <motion.div
              ...
              style={{
                width: SUB_PLANET_SIZE,
                height: SUB_PLANET_SIZE,
                top: y,
                left: x,
              }}
            >
              <div className="w-full h-full rounded-full bg-card border-2 border-border group-hover:border-primary/40 shadow-md flex flex-col items-center justify-center transition-all duration-200">
...
const SafEcosystem = () => {
  ...
  const [subEcosystemId, setSubEcosystemId] = useState<string | null>(null);
  const pick = <T,>(es: T, en?: T): T => (lang === "en" && en ? en : es);
  const selectedModule = modules.find((m) => m.id === selected);
  const subEcosystemModule = modules.find((m) => m.id === subEcosystemId);
  ...
  return (
    <div className="w-full">
      ...
      <AnimatePresence mode="wait">
        {subEcosystemId && subEcosystemModule ? (
          <SubEcosystem
            key="sub-ecosystem"
            parentModule={subEcosystemModule}
            onBack={() => setSubEcosystemId(null)}
          />
        ) : !selected ? (
          <motion.div
            key="ecosystem"
          ...
          >
            ...
            <div className="relative mx-auto" style={{ width: (ORBIT_RADIUS + PLANET_SIZE) * 2, height: (ORBIT_RADIUS + PLANET_SIZE) * 2 }}>
              ...
              {modules.map((mod, i) => {
                ...
                const hasSubEcosystem = !!mod.subEcosystem;
                ...
                return (
                  ...
                  >
                    <div className={`w-full h-full rounded-full bg-card border-2 ${hasSubEcosystem ? "border-primary/30 ring-2 ring-primary/10" : "border-border"} group-hover:border-primary/40 shadow-md group-hover:shadow-lg group-hover:shadow-primary/10 flex flex-col items-center justify-center transition-all duration-200`}>
                    ...
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                      <span className="text-[9px] bg-foreground text-background px-2 py-1 rounded-md font-medium shadow-lg">
                        {hasSubEcosystem
                          ? pick("Clic para ver sub-ecosistema", "Click to see sub-ecosystem")
